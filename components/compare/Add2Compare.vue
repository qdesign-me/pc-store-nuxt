<template>
  <button
    ...
    :class="{ '!text-blue': store.$state.compare.includes(props.productID) }"
    @click="store.toggleCompare(props.productID)"
  >
    <CompareIcon />
    Сравнить
  </button>
  <button v-else class="ml-auto is-transparent p-2 cursor-pointer" :class="{ 'text-blue': store.$state.compare.includes(props.productID) }">
