...
export default defineEventHandler(async (event) => {
  ...
  const andFilters = Object.entries(
    ...
    .map((row) => {
      ...
      return `and productID in (select fop.productID 
    from site_features_on_products fop 
    join site_products_features spf on spf.featureID = fop.featureID and spf.alias='${field}' 
    where ${condition})`;
    })
    .join(' ');
